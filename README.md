...
exist are emitted. `Article` is rendered by `/guides/[slug]`, where there are
real articles to describe; its `author` is an `Organization` for the house
byline rather than a `Person`, and `reviewedBy` is emitted only when a named,
licensed reviewer is on record. The `DiscussionForumPosting` and `ProfilePage`
...
- **Fixture data is fenced off.** Every placeholder surface carries a visible
  notice, *commissioned* guide titles are non-interactive because no article
  exists behind them yet, and the sample Lost & Found reports are non-clickable
  with no contact details — nobody can be sent searching for an animal that does
  not exist. Deleting three files removes all demo content. Real articles live
  in a separate registry and are never mixed into these lists.
- **No invented bylines or credentials.** The byline is a house identity that
  says it is a team of writers, not veterinarians. The reviewer register is
  empty, and the template has no code path that renders a review credit without
  an entry in it — so "vet reviewed" cannot appear until a named, licensed
  reviewer really has read the piece. A test scans every article body for
  claims of professional scrutiny and for figures quoted without a source.
